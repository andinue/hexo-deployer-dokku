...
module.exports = function(args) {
  ...
  const deployDir = pathFn.join(baseDir, '.deploy_dokku');
  ...
  return fs.exists(deployDir).then(exist => {
    ...
    log.info('Setting up Dokku deployment...');
    return setup();
  }).then(() => {
    log.info('Clearing .deploy_dokku folder...');
    ...
    if(!fs.existsSync(`${deployDir}/.static`)){
      fs.writeFileSync(`${deployDir}/.static`, '');
    }
    return;
  }).then(() => {
